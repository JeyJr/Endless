...
public class PanelPlayerInfo : MonoBehaviour
{
    ...
    public void PanelPlayerInfoIsActive()
    {
        ...
        txtSpeed.text = $"<color=green>b:-{gd.bonusAtkSpeed}</color> | <color=blue>a:{gd.agi}</color> | <color=red>w:{gd.weaponSpeedAtk.ToString("F2")}</color> [{gd.AtkSpeed.ToString("F2")}s]";
        txtRange.text = $"<color=green>b:{gd.bonusRangeAtk}</color> | <color=red>w:{gd.weaponRangeAtk}</color> [{gd.RangeAtk}m]";
        txtCritical.text = $"<color=green>b:{gd.bonusCritical}</color> |<color=blue>a:{gd.cri}%</color> | <color=red>w:{gd.weaponCritical}%</color> [{gd.CriticalDMG}%]";
